...
public class OffHeapMap<K, V> extends AbstractMap<K, V> implements AutoCloseable {
    ...
    public OffHeapMap(OffHeapStorage storage, Serializer<K> keySerializer, Serializer<V> valueSerializer, int initialCapacity, boolean threadSafe) {
                ...
                return new Iterator<>() {
                    ...
                    private void advance() {
                        ...
                        if (offset != -1L) {
                            ...
                            byte[] valueData = new byte[valueLength];
                            buffer.get(valueData);
                            nextEntry = new SimpleEntry<>(keySerializer.deserialize(keyData), valueSerializer.deserialize(valueData));
                            offset = buffer.getLong((int) offset + 4);
                        } else {
                            nextEntry = null;
                        }
                    }
